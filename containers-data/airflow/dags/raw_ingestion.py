...
with airflow.DAG('raw_ingestion',
    ...
    task_spark_raw_ingestion_corredor = SSHOperator(
        ...
        command = 'export SPARK_HOME=/usr/local/spark; /opt/conda/bin/spark-submit --master local[*] --executor-memory 4g /opt/shared/scripts/raw_ingestion.py {{ ds }} corredor'
    )

    task_spark_raw_ingestion_empresa = SSHOperator(
        task_id='spark_raw_ingestion_empresa',
        ssh_conn_id='spark_ssh_default',  # Specify your Spark cluster SSH connection ID
        command = 'export SPARK_HOME=/usr/local/spark; /opt/conda/bin/spark-submit --master local[*] --executor-memory 4g /opt/shared/scripts/raw_ingestion.py {{ ds }} empresa'
    )

    task_spark_raw_ingestion_posicao = SSHOperator(
        task_id='spark_raw_ingestion_posicao',
        ssh_conn_id='spark_ssh_default',  # Specify your Spark cluster SSH connection ID
        command = 'export SPARK_HOME=/usr/local/spark; /opt/conda/bin/spark-submit --master local[*] --executor-memory 4g /opt/shared/scripts/raw_ingestion.py {{ ds }} posicao'
    )

    task_spark_raw_ingestion_previsao = SSHOperator(
        task_id='spark_raw_ingestion_previsao',
        ssh_conn_id='spark_ssh_default',  # Specify your Spark cluster SSH connection ID
        command = 'export SPARK_HOME=/usr/local/spark; /opt/conda/bin/spark-submit --master local[*] --driver-cores 6 --executor-cores 6 --driver-memory 16G --executor-memory 16G /opt/shared/scripts/raw_ingestion.py {{ ds }} previsao',
    ...
    [task_spark_raw_ingestion_corredor, task_spark_raw_ingestion_empresa, task_spark_raw_ingestion_posicao, task_spark_raw_ingestion_previsao] >> task_trigger_stage_processing